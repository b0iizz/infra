...
}:
{
  ...
  flake.modules.nixos.yazi =
    { pkgs, ... }:
    {
      environment.systemPackages = [ self.packages.${pkgs.stdenv.hostPlatform.system}.yazi-wrapped ];
          ...
          settings = {
            ...
            program_options = {
              file_manager =
                ...
                "${lib.getExe alacritty-wrapped} -e ${lib.getExe yazi-wrapped} ";
            };
          };
        };
      };
  ...
  perSystem =
    { pkgs, lib, ... }:
    {
      packages.yazi-wrapped = slib.infuse pkgs.yazi {
        ...
        __output.buildCommand = [
          (lib.splitString "\n")
          (map (
            line:
            if lib.hasInfix "makeWrapper" line then
              lib.concatLines [
                line
                #Force sixel adapter on alacritty
                "--run ${lib.escapeShellArg "export TERM=$([[ $TERM -eq \"alacritty\" ]] && echo \"foot\" || echo \"$TERM\" )"} \\"
              ]
              + "\\"
            else
              line
